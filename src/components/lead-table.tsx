    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center h-64 text-muted-foreground gap-2">
          ...
          <p className="text-sm">Run the scraper to find leads or add them manually</p>
        </CardContent>
      </Card>
    )
  }
              ...
              {leads.map((lead) => (
                <tr
                  ...
                  className={`border-b border-border/50 cursor-pointer hover:bg-muted/50 transition-colors ${
                    selectedId === lead.id ? "bg-muted/80" : ""
                  }`}
                >
                  ...
                  <td className="p-3">
                    <div>
                      ...
                      {lead.website && (
                        <p className="text-xs text-muted-foreground truncate max-w-48">{lead.website}</p>
                      )}
                    </div>
                  </td>
                  <td className="p-3">
                    <div className="flex gap-1 flex-wrap">
                      {lead.currencies_used?.map(c => (
                  ...
                  <td className="p-3 text-sm text-muted-foreground">
                    {lead.revenue_estimate
                      ? `€${(lead.revenue_estimate / 1_000_000).toFixed(1)}M`
                      : "—"}
                  </td>
                  <td className="p-3">
